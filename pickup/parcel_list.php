...
<html lang="en">
  ...
  <body>
			<!---Count -->
			<?php
		$sql="SELECT count(id) as total FROM `parcel`";
		$result=mysqli_query($conn,$sql);
		if($result){
			while($row=mysqli_fetch_assoc($result)){
				$total=$row['total'];

				echo '<p style="color:red;"><b>Number of parcel: '.$total.'</b><p>';
			}
		}
		?>
		<!---/Count-->
		<button class="btn btn-primary my-5">
			<a href="./index.php?page=new_parcel" class="text-light">Add New Parcel</a>
		</button>
    <!-- <h1>Hello, world!</h1> -->
