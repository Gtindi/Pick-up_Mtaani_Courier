...
<body>
	<div class="container">
		<!-- <button class="btn btn-primary my-5">
			...
			<!---Count -->
			<?php
		$sql="SELECT count(status) as total FROM `parcel` WHERE status LIKE '%Out for delivery%';";
		$result=mysqli_query($conn,$sql);
		if($result){
			while($row=mysqli_fetch_assoc($result)){
				$total=$row['total'];

				echo '<p style="color:red;"><b>Number of parcel Out for delivery: '.$total.'</b><p>';
			}
		}
		?>
		<!---/Count-->
