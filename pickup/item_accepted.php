...
<body>
	<div class="container">
		<!-- <button class="btn btn-primary my-5">
			...
			<!---Count -->
		<?php
		$sql="SELECT count(status) as total FROM `parcel` WHERE status LIKE '%Item accepted by Pick-up Mtaani%';";
		$result=mysqli_query($conn,$sql);
		if($result){
			while($row=mysqli_fetch_assoc($result)){
				$total=$row['total'];

				echo '<p style="color:red;"><b>Number of parcel accepted by Pick-up Mtaani: '.$total.'</b><p>';
			}
		}
		?>
		<!---/Count-->
...
if(!$conn){
	...
	$dis = 'Item accepted by Pick-up Mtaani';
